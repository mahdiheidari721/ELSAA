...
def inspect_random_race_query_bucket(
    ...
    last_q valid positions, and print bucket-probability diagnostics for RACE layers.
    """
    model.eval()
...
# ==================================================
# ELSAA fusion fix: shared Q/K/V, single output projection
# ==================================================
# ORIGINAL BUG (now removed): HyperRaceGatedAttention / a prior
# HyperRaceMExactAttention / HyperRaceLambdaDependentAttention each wrapped a full
# HyperLSHExactAttention and a full RACEAttention as independent sub-modules. Each
# of those owns its OWN q/k/v Linear layers AND its OWN output Linear layer (self.o),
# applied INSIDE the branch BEFORE the two branch outputs were gated and summed.
#
# ELSAA Algorithm 1 specifies:
#   Q,K,V <- X W_Q, X W_K, X W_V                      (ONE shared projection)
#   O_sp, d_sp <- sortLSH(Q,K,V);  O_lr, d_lr <- RACE(Q,K,V)
#   O_head <- g_sp * m_sp * O_sp + g_lr * O_lr        (fuse in per-head space)
#   O <- O_head @ W_O                                  (ONE shared output projection,
#                                                        applied AFTER fusion)
#
# The classes below implement that faithfully: ONE q,k,v,o Linear set per attention
# module; both branches are "headless" functions that consume/produce raw [B,H,T,Dh]
# tensors with no projections of their own; fusion happens before the single self.o.

def headless_sortlsh_branch(Qh, Kh, Vh, lsh: AngularLSHGray, block_size, min_seq_len,
                             neighbor_blocks, mask=None):
    """Qh,Kh,Vh: [B,H,T,Dh]. Returns (O [B,H,T,Dh], log_d [B,H,T])."""
    B, H, T, D = Qh.shape
    out = torch.zeros_like(Qh)
    logd = torch.full((B, H, T), float("-inf"), device=Qh.device, dtype=Qh.dtype)
    ...
    for b in range(B):
        ...
        if valid_T == 0:
            continue
        q, k, v = Qh[b, :, :valid_T, :], Kh[b, :, :valid_T, :], Vh[b, :, :valid_T, :]

        if valid_T < min_seq_len:
            o = _run_exact_sdpa(q.unsqueeze(0), k.unsqueeze(0), v.unsqueeze(0))[0]
            with torch.no_grad():
                logits = torch.einsum("hqd,hkd->hqk", q, k) * scale
                lse = torch.logsumexp(logits.float(), dim=-1).to(q.dtype)
            out[b, :, :valid_T, :] = o
            logd[b, :, :valid_T] = lse
            continue
        ...
        qb = lsh.hash(q)
        kb = lsh.hash(k)
        q_order = torch.argsort(qb, dim=1, stable=True)
        k_order = torch.argsort(kb, dim=1, stable=True)
        q_inv = torch.argsort(q_order, dim=1, stable=True)

        qs = _gather_tokens_3d(q, q_order)
        ks = _gather_tokens_3d(k, k_order)
        vs = _gather_tokens_3d(v, k_order)
        ...
        rem = Tv % bsz
        out_sorted = torch.zeros_like(qs)
        lse_sorted = torch.empty(Hh, Tv, device=qs.device, dtype=qs.dtype)

        if neighbor_blocks == 0:
            if nfull > 0:
                Tf = nfull * bsz
                Qf = qs[:, :Tf, :].reshape(Hh, nfull, bsz, Dd)
                Kf = ks[:, :Tf, :].reshape(Hh, nfull, bsz, Dd)
                Vf = vs[:, :Tf, :].reshape(Hh, nfull, bsz, Dd)
                Qfl = Qf.reshape(Hh * nfull, 1, bsz, Dd)
                Kfl = Kf.reshape(Hh * nfull, 1, bsz, Dd)
                Vfl = Vf.reshape(Hh * nfull, 1, bsz, Dd)
                Ofl = _run_exact_sdpa(Qfl, Kfl, Vfl)
                out_sorted[:, :Tf, :] = Ofl.reshape(Hh, nfull, bsz, Dd).reshape(Hh, Tf, Dd)
                with torch.no_grad():
                    chunks = []
                    for s in range(0, nfull, 32):
                        e = min(s + 32, nfull)
                        lg = torch.einsum("hnqd,hnkd->hnqk", Qf[:, s:e], Kf[:, s:e]) * scale
                        chunks.append(torch.logsumexp(lg.float(), dim=-1).to(qs.dtype))
                    lse_sorted[:, :Tf] = torch.cat(chunks, dim=1).reshape(Hh, Tf)
            if rem > 0:
                ql, kl, vl = qs[:, nfull*bsz:, :], ks[:, nfull*bsz:, :], vs[:, nfull*bsz:, :]
                ol = _run_exact_sdpa(ql.unsqueeze(0), kl.unsqueeze(0), vl.unsqueeze(0))[0]
                out_sorted[:, nfull*bsz:, :] = ol
                with torch.no_grad():
                    lg = torch.einsum("hqd,hkd->hqk", ql, kl) * scale
                    lse_sorted[:, nfull*bsz:] = torch.logsumexp(lg.float(), dim=-1).to(qs.dtype)
        else:
            nb = math.ceil(Tv / bsz)
            for bi in range(nb):
                q0, q1 = bi * bsz, min((bi + 1) * bsz, Tv)
                left = max(0, bi - neighbor_blocks)
                right = min(nb - 1, bi + neighbor_blocks)
                k0, k1 = left * bsz, min((right + 1) * bsz, Tv)
                qblk, kblk, vblk = qs[:, q0:q1], ks[:, k0:k1], vs[:, k0:k1]
                oblk = _run_exact_sdpa(qblk.unsqueeze(0), kblk.unsqueeze(0), vblk.unsqueeze(0))[0]
                out_sorted[:, q0:q1, :] = oblk
                with torch.no_grad():
                    lg = torch.einsum("hqd,hkd->hqk", qblk, kblk) * scale
                    lse_sorted[:, q0:q1] = torch.logsumexp(lg.float(), dim=-1).to(qs.dtype)

        o_unsorted = out_sorted.gather(1, q_inv.unsqueeze(-1).expand(-1, -1, Dd))
        lse_unsorted = lse_sorted.gather(1, q_inv)
        out[b, :, :valid_T, :] = o_unsorted
        logd[b, :, :valid_T] = lse_unsorted

    return out, logd


def headless_race_branch(Qh, Kh, Vh, ace: BatchedACE, mask=None, eps=1e-6):
    """Qh,Kh,Vh: [B,H,T,Dh]. Returns (O [B,H,T,Dh], d_race [B,H,T])."""
    B, H, T, D = Qh.shape
    M = ace.M

    def pack(z):
        return z.permute(0, 2, 1, 3).unsqueeze(0).expand(M, -1, -1, -1, -1)  # [M,B,T,H,D]

    Khf, Vhf, Qhf = pack(Kh), pack(Vh), pack(Qh)
    out_m = ace(Khf, Vhf, Qhf, eps=eps)         # [M,B,H,T,D]
    out = out_m.mean(dim=0)                     # [B,H,T,D]

    scale = ace.logit_temp.exp().clamp(1e-2, 20.0)
    if ace.share_planes:
        N = M * Khf.shape[1] * H
        Kh2 = Khf.permute(0, 1, 3, 2, 4).contiguous().view(N, T, D)
        Qh2 = Qhf.permute(0, 1, 3, 2, 4).contiguous().view(N, T, D)
        projK = Kh2 @ ace.planes_T
        projQ = Qh2 @ ace.planes_T
    else:
        BH = Khf.shape[1] * H
        Kh2 = Khf.permute(0, 1, 3, 2, 4).contiguous().view(M, BH, T, D)
        Qh2 = Qhf.permute(0, 1, 3, 2, 4).contiguous().view(M, BH, T, D)
        projK = torch.einsum("mbtd,mds->mbts", Kh2, ace.planes_T).view(M * BH, T, ace.L * ace.K)
        projQ = torch.einsum("mbtd,mds->mbts", Qh2, ace.planes_T).view(M * BH, T, ace.L * ace.K)
        N = M * BH
    ...
    probsK = F.softmax(logitsK, dim=-1).view(N, T, ace.L * ace.R)
    probsQ = F.softmax(logitsQ, dim=-1).view(N, T, ace.L * ace.R)

    if mask is not None:
        mask_bh = mask[:, None, :].expand(Khf.shape[1], H, T).reshape(-1, T)
        mask_N = mask_bh.unsqueeze(0).expand(M, -1, -1).reshape(N, T).to(probsK.dtype)
        probsK = probsK * mask_N.unsqueeze(-1)
        probsQ = probsQ * mask_N.unsqueeze(-1)

    bucket_mass = probsK.sum(dim=1)                                          # [N,S]
    d_race = torch.einsum("nts,ns->nt", probsQ, bucket_mass).clamp_min(eps)  # [N,T]
    d_race = d_race.view(M, Khf.shape[1], H, T).mean(dim=0)                  # [B,H,T]

    return out, d_race


class SharedQKVELSAAAttention(nn.Module):
    """
    Correct ELSAA fusion with a SINGLE q,k,v,o projection set shared by both branches.

    lambda_mode:
      "none"      -> m_sparse forced to 1 (this is the Sort_Lsh_RACE ablation, now
                      with correctly-shared QKV/O, which the old HyperRaceGatedAttention
                      did not have either)
      "scalar"    -> lambda = exp(log_lambda), one learnable scalar per layer
      "dependent" -> lambda_i = c + sigmoid(w^T x_i + b), query/token-dependent
    """

    def __init__(self, cfg, device=DEVICE, lambda_mode="scalar"):
        super().__init__()
        d, h, drop = cfg["embed_dim"], cfg["num_heads"], cfg["drop_rate"]
        qkv_bias = cfg["qkv_bias"]
        self.h, self.dk = h, d // h
        self.lambda_mode = lambda_mode
        self.eps = cfg.get("mexact_eps", 1e-6)

        # the ONE shared projection set (this replaces self.hyper.{q,k,v,o} and
        # self.race.{q,k,v,o} from the old buggy implementation)
        self.q = nn.Linear(d, d, bias=qkv_bias)
        self.k = nn.Linear(d, d, bias=qkv_bias)
        self.v = nn.Linear(d, d, bias=qkv_bias)
        self.o = nn.Linear(d, d)
        ...
        self.lsh = AngularLSHGray(num_bits=cfg.get("hyper_num_bits", 5), dim=self.dk, device=device)
        self.block_size = cfg.get("hyper_block_size", 256)
        self.min_seq_len = cfg.get("hyper_min_seq_len", 4096)
        self.neighbor_blocks = cfg.get("hyper_neighbor_blocks", 0)

        self.ace = BatchedACE(self.dk, cfg["K"], cfg["L"], cfg["M"], device=device)

        if lambda_mode == "scalar":
            init = float(cfg.get("mexact_lambda_init", 1.0))
            self.log_lambda = nn.Parameter(torch.tensor(math.log(init), dtype=torch.float32))
        elif lambda_mode == "dependent":
            std = float(cfg.get("mexact_dependent_lambda_w_init_std", 1e-3))
            self.lambda_w = nn.Parameter(torch.empty(d).normal_(0, std))
            offset = float(cfg.get("mexact_dependent_lambda_offset", 0.3))
            self.lambda_offset = nn.Parameter(torch.tensor(offset, dtype=torch.float32))
            target = float(cfg.get("mexact_dependent_lambda_init_target", offset + 0.5))
            p = min(max(target - offset, 1e-4), 1 - 1e-4)
            self.lambda_bias = nn.Parameter(torch.tensor(math.log(p / (1 - p)), dtype=torch.float32))
        elif lambda_mode != "none":
            raise ValueError(f"Unknown lambda_mode: {lambda_mode}")

        gate_hidden = cfg.get("gate_hidden_dim", 64)
        self.gate_mlp = nn.Sequential(nn.Linear(d, gate_hidden), nn.SiLU(), nn.Linear(gate_hidden, 2))
        self.normalize_gates = cfg.get("gate_normalize", False)
    ...
    def forward(self, x, mask=None):
        ...
        O_sp, log_d_sp = headless_sortlsh_branch(
            Q, K, V, self.lsh, self.block_size, self.min_seq_len, self.neighbor_blocks, mask
        )
        O_lr, d_lr = headless_race_branch(Q, K, V, self.ace, mask, eps=self.eps)

        log_lam = None
        if self.lambda_mode == "none":
            m_sp = torch.ones_like(log_d_sp)
        else:
            log_d_sp_d = log_d_sp.detach().float()
            log_d_lr_d = torch.log(d_lr.detach().float().clamp_min(self.eps))
            log_eps = torch.full_like(log_d_sp_d, math.log(self.eps))

            if self.lambda_mode == "scalar":
                log_lam = self.log_lambda.float()
            else:  # dependent: lambda_i = c + sigmoid(w^T x_i + b), broadcast over heads
                logits = torch.matmul(x.float(), self.lambda_w.float()) + self.lambda_bias.float()
                lam = self.lambda_offset.float().clamp_min(0.0) + torch.sigmoid(logits)  # [B,T]
                lam = lam.clamp_min(1e-6).unsqueeze(1).expand(-1, H, -1)                  # [B,H,T]
                log_lam = torch.log(lam)

            log_den = torch.logsumexp(
                torch.stack([log_d_sp_d, log_lam + log_d_lr_d, log_eps], dim=0), dim=0
            )
            m_sp = torch.exp(log_d_sp_d - log_den).to(Q.dtype)   # [B,H,T]

        g = torch.sigmoid(self.gate_mlp(x))  # [B,T,2]  (gate from token reps, matches your convention)
        if self.normalize_gates:
            g = g / (g.sum(-1, keepdim=True) + 1e-6)
        g_sp = g[..., 0].unsqueeze(1).expand(-1, H, -1)  # [B,H,T]
        g_lr = g[..., 1].unsqueeze(1).expand(-1, H, -1)

        # *** fuse in head-space, BEFORE the single shared output projection ***
        O_head = g_sp.unsqueeze(-1) * m_sp.unsqueeze(-1) * O_sp + g_lr.unsqueeze(-1) * O_lr  # [B,H,T,D]

        out = O_head.transpose(1, 2).contiguous().view(B, T, H * D)  # [B,T,d]
        out = self.drop(out)
        out = self.o(out)                                            # *** single shared W_O ***
        if mask is not None:
            out = out * mask.unsqueeze(-1).to(out.dtype)

        with torch.no_grad():
            self.last_gates = g.detach()
            self.last_m_sparse = m_sp.detach()
            if self.lambda_mode == "scalar":
                self.last_lambda = self.log_lambda.detach().exp()
            elif self.lambda_mode == "dependent" and log_lam is not None:
                self.last_lambda = log_lam.detach().exp()
            if mask is not None and bool(mask.any()):
                valid = mask.bool().unsqueeze(1).expand(-1, H, -1)
                self.last_d_sparse_mean = torch.exp(log_d_sp.detach().clamp(max=20))[valid].mean()
                self.last_d_race_mean = d_lr.detach()[valid].mean()
        return out


class SharedQKVELSAABlock(nn.Module):
    def __init__(self, cfg, device=DEVICE, lambda_mode="scalar"):
        super().__init__()
        d, drop = cfg["embed_dim"], cfg["drop_rate"]
        self.att = SharedQKVELSAAAttention(cfg, device=device, lambda_mode=lambda_mode)
        self.norm1, self.norm2 = nn.LayerNorm(d), nn.LayerNorm(d)
        self.ff = nn.Sequential(nn.Linear(d, cfg["mlp_dim"]), nn.GELU(), nn.Linear(cfg["mlp_dim"], d))
        self.drop = nn.Dropout(drop)
...
class TextTransformerClassifier(nn.Module):
    def __init__(self, cfg, attn_type: str):
        ...
        # --- FIXED ELSAA fusion: shared Q/K/V, single output projection ---
        elif attn_type == "hyper_race_mexact":
            Block = lambda c: SharedQKVELSAABlock(c, device=DEVICE, lambda_mode="scalar")
        elif attn_type in {"hyper_race_lambda_dependent", "hyper_race_dependent_lambda"}:
            Block = lambda c: SharedQKVELSAABlock(c, device=DEVICE, lambda_mode="dependent")
        elif attn_type == "hyper_race":
            # Sort_Lsh_RACE ablation (m_sparse == 1), now also with shared QKV/O
            Block = lambda c: SharedQKVELSAABlock(c, device=DEVICE, lambda_mode="none")
        # --------------------------------------------------------------------
...
def _collect_hybrid_attention_stats(model, attn_type, stats):
    """Updated for the fixed SharedQKVELSAAAttention, which is now used for
    "hyper_race" (lambda_mode='none'), "hyper_race_mexact" (lambda_mode='scalar'),
    and "hyper_race_lambda_dependent" (lambda_mode='dependent')."""
    if attn_type not in {
        "hyper_race",
        "hyper_race_mexact",
        "hyper_race_lambda_dependent",
    ...
    for layer_idx, layer in enumerate(model.layers):
        ...
        if hasattr(att, "last_gates") and att.last_gates is not None:
            ...
            _update_running_stat(stats, f"gates/layer{layer_idx}_sparse", gates[..., 0])
            _update_running_stat(stats, f"gates/layer{layer_idx}_lowrank", gates[..., 1])

        if hasattr(att, "last_m_sparse") and att.last_m_sparse is not None:
            _update_running_stat(stats, f"m_sparse/layer{layer_idx}", att.last_m_sparse)

        if hasattr(att, "last_d_sparse_mean") and att.last_d_sparse_mean is not None:
            _update_running_stat(stats, f"den/layer{layer_idx}_sparse", att.last_d_sparse_mean)
        ...
        if hasattr(att, "last_lambda") and att.last_lambda is not None:
            _update_running_stat(stats, f"lambda/layer{layer_idx}", att.last_lambda)
